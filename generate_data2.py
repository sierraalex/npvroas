...
def get_incremental_npv(spend_df, channel, delta_spend_annual, personas, channels, mmm_params,
                        months_horizon, discount_rate_annual, seed=42, n_sims=50):
    baseline_npvs = []
    test_npvs = []
    ...
    for sim in range(n_sims):
        ...
        baseline_spend_df = spend_df.copy()
    ...
    npv_deltas = np.array(test_npvs) - np.array(baseline_npvs)
    incremental_npv_std = np.std(npv_deltas, ddof=1)

    return (
    incremental_npv_after_spend,
    avg_baseline_npv,
    avg_test_npv,
    incremental_npv_std
)
...
final_test_amount = 50000

print("\n--- FINAL CHANNEL RANKING TEST (+$50,000 ANNUAL, net of spend) ---")
...
for ch in CHANNELS.keys():
    ...
    incremental_npv, avg_baseline_npv, avg_test_npv, incremental_npv_std = get_incremental_npv(
        spend_df,
        ch,
        final_test_amount,
        PERSONAS,
    ...
    'channel': ch,
    'incremental_npv_50k': incremental_npv,
    'incremental_npv_std_50k': incremental_npv_std,
    'risk_adjusted_npv': incremental_npv / incremental_npv_std if incremental_npv_std > 0 else np.nan
})
...
final_results_df.to_csv(os.path.join(output_dir, "final_channel_ranking.csv"), index=False)
